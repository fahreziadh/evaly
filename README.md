...
### We are currenlty migrate to convex and tanstack start, you can see the progress on this PR [#40](https://github.com/fahreziadh/evaly/pull/40)
...
```bash
bunx --bun run build || bun run build
```
...
```bash
bunx --bun run test || bun run test
```
