...
export const ExportDialogDetails = ({
  ...
  const prepareQuestionsData = () => {
    return questions.map((question, index) => {
      ...
      return {
        ...
        ParticipantAnswer:
          question.participantAnswer?.replace(/<[^>]*>?/gm, '') || 'Not answered',
        Status:
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(12)
      doc.text(
        `Participant: ${submissionData.Name} (${submissionData.Email})`,
        14,
        yPos
      )
      yPos += 7
      doc.text(
        `Score: ${submissionData.Score} | Rank: ${submissionData.Rank}`,
        14,
        yPos
      )
      yPos += 7
      ...
      if (sectionsData.length > 0) {
        ...
        const lastTableY = (doc as unknown as { lastAutoTable: { finalY: number } })
      ...
      if (questionsData.length > 0) {
        ...
        autoTable(doc, {
          ...
          body: questionsData.map(item => [
            ...
            item.Status,
            item.ParticipantAnswer.substring(0, 50) +
              (item.ParticipantAnswer.length > 50 ? '...' : '')
          ]),
          startY: yPos,
          styles: { fontSize: 8, cellPadding: 2 },
          columnStyles: {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="text-muted-foreground text-xs">
          Note: The export will include submission summary, section performance, and
          question details.
        </div>
      </DialogContent>
    </Dialog>
